...
WHISPER_DIR = os.path.join(MODELS_DIR, "whisper", "faster-whisper-small")
PYANNOTE_DIR = os.path.join(MODELS_DIR, "pyannote", "speaker-diarization-community-1")
...
# =========================
# WHISPER TRANSKRIPTION (LOKALES MODELL)
# =========================
def transcribe_whisper(audio_path, language="de", device="cpu"):
    if not os.path.isdir(WHISPER_DIR):
        raise RuntimeError(f"Whisper-Modellverzeichnis nicht gefunden: {WHISPER_DIR}")

    print(f"Lade Whisper-Modell aus: {WHISPER_DIR}")
    model = WhisperModel(WHISPER_DIR, device=device)
...
# =========================
# PYANNOTE DIARISIERUNG (LOKALES MODELL)
# =========================
def diarize_speakers(audio_path):
    if not os.path.isdir(PYANNOTE_DIR):
...
# =========================
# VERARBEITUNG EINER AUDIODATEI (KERNLOGIK)
# =========================
